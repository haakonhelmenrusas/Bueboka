...
describe('PublicProfileList', () => {
  describe('loading state', () => {
    it('shows a loading spinner and text', () => {
      const { getByText } = render(
        <PublicProfileList profiles={[]} loading={true} searched={false} query="" />,
      );
    ...
    it('does not show results while loading', () => {
      const { queryByText } = render(
        <PublicProfileList profiles={[alice]} loading={true} searched={false} query="" />,
      );
  ...
  describe('idle state (before first search)', () => {
    it('renders nothing when not yet searched', () => {
      const { toJSON } = render(
        <PublicProfileList profiles={[]} loading={false} searched={false} query="" />,
      );
  ...
  describe('empty results', () => {
    it('shows "Ingen resultater" heading', () => {
      const { getByText } = render(
        <PublicProfileList profiles={[]} loading={false} searched={true} query="xyz" />,
      );
  ...
  describe('results', () => {
    it('renders a profile card with name and club', () => {
      const { getByText } = render(
        <PublicProfileList profiles={[alice]} loading={false} searched={true} query="alice" />,
      );
    ...
    it('renders all profiles when multiple are returned', () => {
      const { getByText } = render(
        <PublicProfileList profiles={[alice, bob]} loading={false} searched={true} query="bue" />,
      );
      expect(getByText('Alice Archer')).toBeTruthy();
    ...
    it('does not crash when a profile has no club', () => {
      const { getByText, queryByText } = render(
        <PublicProfileList profiles={[noClub]} loading={false} searched={true} query="charlie" />,
      );
